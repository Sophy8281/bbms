...
<div class="panel panel-default">
    <div class="panel-heading"><h5>Issue Platelet Bag</h5></div>
        ...
        <form action="{{ url('staff/platelet/issue/'.$platelet->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <p class="hint--top" data-hint="Blood Group" id="input-field">
                    ...
                    <input type="text" name="group_id" class="form-control @error('group_id') is-invalid @enderror" value="{{ $platelet->group->name }}" readonly>
                    @error('group_id')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            @else
            <p class="bg-warning p-3 text-center">There are no requests for platelets with blood group {{ $platelet->group->name }}</p>
            @endif
